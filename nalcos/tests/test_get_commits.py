...
def test_get_local_commits():
    assert get_local_commits("nalcos_pytest", 10, "pytest") == COMMITS


def test_get_local_commits_exception():
    with pytest.raises(BranchNotFoundException):
        get_local_commits("nalcos_pytest", 10, "no-branch")


def test_get_github_commits():
    assert get_github_commits("thepushkarp/nalcos", 10, "pytest") == COMMITS


def test_get_github_commits_exception():
    with pytest.raises(BranchNotFoundException):
        get_github_commits("thepushkarp/nalcos", 10, "no-branch")
